...
public:
    vector<int> minInterval(vector<vector<int>>& intervals, vector<int>& queries) {
        ...
        for (int i = 0; i < queries.size(); i++) qs.push_back({queries[i], i}); // sorted queries

        sort(intervals.begin(), intervals.end());
        sort(qs.begin(), qs.end());

        priority_queue<pair<int,int>, vector<pair<int,int>>, greater<pair<int,int>>> pq; // {length, endtime}
